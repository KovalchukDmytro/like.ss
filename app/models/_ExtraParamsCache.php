...
class ExtraParamsCache extends \yii\db\ActiveRecord
{
    ...
    public function rules()
    {
        return [
            ...
            [['value_name_1', 'value_alt_name'], 'string', 'max' => 255],
    ...
    public function attributeLabels()
    {
        return [
            ...
            'value_alt_name' => Yii::t('app', 'Value Alt Name'),
    ...
    public static function getCategoryFilter($ids, $selected)
    {
        ...
        {
            $selected = explode('-or-', (string)$selected);
//            $selected = array_map('intval', $selected);
        ...
        $lang = (int)\app\models\Lang::getCurrentId();
            ...
            if (is_array($res) && count($res))
            {
                ...
                {
                    $url_params['filter'] = explode('-or-', $url_params['filter']);
                ...
                foreach ($res as $row)
                {
                    ...
                    $row['selected'] = in_array($row['value_alt_name'], $selected) ? true : false;
                    //	Формирую ссылку для значения
                    $url = $url_params;
                    //	Добавляю или удаляю id значения из параметра filter
                    if (isset($url['filter'][$row['value_alt_name']]))
                    {
                        unset($url['filter'][$row['value_alt_name']]);
                    }
                    else
                    {
                        $url['filter'][$row['value_alt_name']] = $row['value_alt_name'];
                    }
                    //	Собираю массив id значений в строку или удаляю параметр вообще, если в нём нет ни одного id
                    if (count($url['filter']))
                    {
                        sort($url['filter']);
                        $url['filter'] = implode('-or-', $url['filter']);
                    }
                    else
                    {
                        unset($url['filter']);
                    ...
                    // это оптимизация(костыль). спасибо, SEO
                    if(isset($url['id']) && isset($url['filter'])) {
                        $url ='/'.$url['alias'].'/'.$url['id'].'/filter/'.$url['filter'];
                    } 
                    elseif(!isset($url['id']) && isset($url['filter'])) {
                        $url ='/'.$url['alias'].'/filter/'.$url['filter'];
                    }
                    elseif(isset($url['id']) && !isset($url['filter'])) {
                        $url = '/'.$url['alias'].'/'.$url['id'];
                    }
                    elseif(!isset($url['id']) && !isset($url['filter'])) {
                        $url = '/'.$url['alias'];
                    }
            ...
            foreach ($filter as $param)
            {
                if (isset($param['values']) && is_array($param['values']) && count($param['values']))
                {
                    foreach ($param['values'] as $value)
                    {
                        ...
                        {
                            $result[$value['value_alt_name']] = $value;
                        }
                    }
                }
            }
